...
impl GridRenderer {
    ...
    pub fn new(env: String, is_watch: bool) -> Arc<Self> {
        ...
        // Filling-bar spinner used as the "alive" indicator on the
        // header line. Cycles ▱▱▱ → ▰▱▱ → ▰▰▱ → ▰▰▰ → ▱▱▱ … at the
        // 250 ms steady tick below, one full sweep per second.
        //
        // indicatif treats the LAST element of `tick_strings` as the
        // "finished" frame (shown only after `finish_*` is called), so
        // we duplicate ▰▰▰ at the tail to make it appear during normal
        // rotation as well as at exit.
