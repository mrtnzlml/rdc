...
/// Compute the combined hash for a rule in codec format (byte-identical to
/// the legacy `rule_combined_hash`).
///
...
pub async fn process(
    ...
    for r in &rules {
        let slug = match ctx.lockfile.slug_for_id(KIND, r.id) {
                    ...
                    let (resolved_json, resolved_code) = if symmetric {
                            ...
                            (&pre_local_code, &proposed_code)
                        {
                            ...
                            Some(String::from_utf8(bytes).with_context(|| {
                                format!("rule code resolved bytes for '{}' are not UTF-8", r.name)
                            })?)
                        } else {
                            None
                        };
