...
pub async fn process(ctx: &mut PullCtx<'_>, listed: MdhListed, progress: &Arc<OverallProgress>) -> Result<(usize, usize)> {
    ...
    for c in collections {
        ...
        let c_recorded = apply_pull_action(c_action, &coll_path, &coll_proposed, c_remote_hash, ctx.interactive, progress, &ctx.env)?;
        record_object(
            ctx.lockfile,
            "mdh_collections",
            &slug,
    ...
    for (slug, dataset_dir, c) in &dataset_dirs {
        ...
        let i_recorded = apply_pull_action(i_action, &ix_path, &ix_proposed, i_remote_hash, ctx.interactive, progress, &ctx.env)?;
        record_object(
            ctx.lockfile,
            "mdh_indexes",
            slug,
