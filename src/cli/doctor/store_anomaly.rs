...
pub fn find_anomalies(paths: &Paths) -> Result<Vec<(String, Hook)>> {
    ...
    for entry in
        std::fs::read_dir(&hooks_dir).with_context(|| format!("reading {}", hooks_dir.display()))?
    {
        let path = entry?.path();
...
pub async fn run(env: &str, check: bool, yes: bool) -> Result<()> {
    ...
    if anomalies.is_empty() {
        log.event(
            Action::Info,
            &format!("no anomalous store-extension hooks in env '{env}'"),
        );
        return Ok(());
    }
    ...
    log.event(
        Action::Info,
        &format!("{} anomalous hook(s) in env '{env}':", anomalies.len()),
    );
    for (slug, hook) in &anomalies {
        log.event(
            Action::Info,
            &format!(
                "  hooks/{slug}  (id {}, name {:?}, type {})",
    ...
    for (slug, hook) in anomalies {
        ...
        match cure {
            ...
            crate::cli::resolve::AnomalyCure::Convert => {
                ...
                lockfile.save(&paths.lockfile()).with_context(|| {
                    format!("saving lockfile to {}", paths.lockfile().display())
                })?;
                fixed += 1;
            }
            crate::cli::resolve::AnomalyCure::Reinstall => {
                reinstall_as_store_extension(&client, &mut lockfile, &paths, &slug, &hook, &log)
                    .await?;
                lockfile.save(&paths.lockfile()).with_context(|| {
                    format!("saving lockfile to {}", paths.lockfile().display())
                })?;
    ...
    log.event(
        Action::Doctor,
        &format!("done env '{env}': {fixed} fixed, {skipped} skipped"),
    );
    Ok(())
}
...
async fn convert_to_custom(
    ...
    log.event(
        Action::Doctor,
        &format!(
            "hooks/{slug} (id {}) \u{2192} converted to custom",
            updated.id
        ),
    );
    Ok(())
...
pub fn match_template<'a>(
    ...
    let matches: Vec<&crate::model::HookTemplate> = templates
        .iter()
        .filter(|t| {
            (
                t.name.as_str(),
                t.template_type.as_str(),
                t.extension_source.as_str(),
            ) == key
        })
        .collect();
    match matches.len() {
...
async fn reinstall_as_store_extension(
    ...
    let templates = client
        .list_hook_templates(Some(log.clone()))
        .await
    ...
    let remote_hooks = client
        .list_hooks(Some(log.clone()))
        .await
    ...
    let installed_id = match crate::cli::deploy::store_extensions::find_orphan(
        ...
        _ => {
            ...
            let installed = client
                .create_hook_via_install(&install_body, Some(log.clone()))
                .await
    ...
    let updated = client
        .update_hook_value(
            installed_id,
            &serde_json::Value::Object(patch_body),
            Some(log.clone()),
        )
        .await
    ...
    for h in &remote_hooks {
        ...
        let new_run_after: Vec<serde_json::Value> = run_after
            .into_iter()
            .map(|v| {
                if v.as_str() == Some(&old_url) {
        ...
        client
            .update_hook_value(h.id, &body, Some(log.clone()))
            .await
    ...
    if rewired > 0 {
        log.event(
            Action::Info,
            &format!("hooks/{slug}: rewired {rewired} dependent(s) to new URL"),
        );
    }
        ...
        .await
    {
        ...
        Err(e) if crate::api::anyhow_has_status(&e, 404) => {
            log.event(
                Action::Info,
                &format!("hooks/{slug}: old id {} already gone", old_hook.id),
            );
        }
    ...
    for h in &remote_hooks {
        ...
        let run_after_had_old = h
            .extra
            .get("run_after")
            .and_then(|v| v.as_array())
            .map(|a| a.iter().any(|v| v.as_str() == Some(&old_url)))
            .unwrap_or(false);
        if !run_after_had_old {
            continue;
        }
        let Some(dep_slug) = lockfile.slug_for_id("hooks", h.id).map(|s| s.to_string()) else {
            continue;
        };
        let fresh = client
            .get_hook(h.id, Some(log.clone()))
            .await
        ...
        let dep_prior = lockfile
            .objects
            .get("hooks")
            .and_then(|m| m.get(&dep_slug))
            .cloned();
        lockfile.upsert(
            "hooks",
            &dep_slug,
